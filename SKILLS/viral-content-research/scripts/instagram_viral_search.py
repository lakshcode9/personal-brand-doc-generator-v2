...
def load_config():
    """Load API tokens and persona hashtags from markdown reference files."""
    ...
    if api_keys_file.exists():
        ...
        for line in content.splitlines():
            if "**Token**:" in line or "- Token:" in line:
                config["apify_token"] = line.split(":")[-1].strip().strip("`").strip()
        ...
        for line in content.splitlines():
            if "## Persona 1" in line:
                ...
                if current_persona == 1:
                    config["persona_1_hashtags"].append(tag)
                else:
                    config["persona_2_hashtags"].append(tag)
...
PERSONA_1_HASHTAGS = CONFIG["persona_1_hashtags"]
PERSONA_2_HASHTAGS = CONFIG["persona_2_hashtags"]
...
def main():
    ...
    # ---- Persona 1 ----
    print("\n" + "=" * 60)
    print("  PERSONA 1: Business Professionals (AI for Work)")
    print("=" * 60)

    raw_p1 = run_hashtag_scraper(PERSONA_1_HASHTAGS, RESULTS_PER_HASHTAG)
    results_p1 = process_results(raw_p1, "Business Professionals (AI for Work)")
    print(f"  Persona 1: {len(results_p1)} reels found")

    # ---- Persona 2 ----
    print("\n" + "=" * 60)
    print("  PERSONA 2: AI Agency Builders")
    print("=" * 60)

    raw_p2 = run_hashtag_scraper(PERSONA_2_HASHTAGS, RESULTS_PER_HASHTAG)
    results_p2 = process_results(raw_p2, "AI Agency Builders")
    print(f"  Persona 2: {len(results_p2)} reels found")

    # ---- Get follower counts ----
    all_results = results_p1 + results_p2
    all_usernames = [r["username"] for r in all_results if r["username"]]
    ...
    # Split back
    results_p1 = [r for r in all_results if r["persona"] == "Business Professionals (AI for Work)"]
    results_p2 = [r for r in all_results if r["persona"] == "AI Agency Builders"]

    # Sort by virality
    results_p1.sort(key=lambda x: x.get("virality_ratio", 0) or 0, reverse=True)
    results_p2.sort(key=lambda x: x.get("virality_ratio", 0) or 0, reverse=True)

    # Filter winners
    winners_p1 = [r for r in results_p1 if (r.get("virality_ratio") or 0) >= 3]
    winners_p2 = [r for r in results_p2 if (r.get("virality_ratio") or 0) >= 3]

    all_data = {
        "persona_1": {"all": results_p1, "winners": winners_p1},
        "persona_2": {"all": results_p2, "winners": winners_p2},
    }
    ...
    for label, winners in [
        ("PERSONA 1: Business Professionals", winners_p1),
        ("PERSONA 2: AI Agency Builders", winners_p2),
    ]:
        print(f"\n\n{'=' * 60}")
        print(f"  TOP WINNERS -- {label}")
        print(f"{'=' * 60}")

        for i, w in enumerate(winners[:15]):
            print(f"\n  #{i+1}")
