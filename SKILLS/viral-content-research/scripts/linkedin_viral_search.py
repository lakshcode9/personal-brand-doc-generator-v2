...
def load_config():
    """Load API tokens and profile URLs from markdown reference files."""
    ...
    # Load Profiles
    creators_file = ref_dir / "creators.md"
    if creators_file.exists():
        content = creators_file.read_text(encoding='utf-8')
        for line in content.splitlines():
            if "](" in line and "linkedin.com/in/" in line:
                url = line.split("](")[-1].split(")")[0].strip()
                config["profile_urls"].append(url)
            elif "linkedin.com/in/" in line and line.strip().startswith("- "):
...
def main():
    ...
    raw_items = run_linkedin_scraper(PROFILE_URLS, posted_limit=20)
